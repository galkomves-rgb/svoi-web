...
export function StatusBadge({ status, visibility, featured }: StatusBadgeProps) {
  return (
    <div className="flex flex-wrap gap-2">
      {status ? <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusStyles[status]}`}>{statusLabel[status]}</span> : null}
      ...
      {featured ? <span className="rounded-full bg-blue-900 px-3 py-1 text-xs font-semibold text-white">Рекомендовано</span> : null}
    </div>
  );
}
