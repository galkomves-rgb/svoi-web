    ...
    <Card as="article" className="space-y-4 rounded-3xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="text-right">
          ...
          <span className="text-sm text-slate-500">{listing.date}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm text-slate-500">
        <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1">{listing.district}</span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
        ...
        <Link href={`/${citySlug}/listings/${listing.id}`} className="cta-secondary">
          Відкрити деталі
        </Link>
      </div>
    </Card>
